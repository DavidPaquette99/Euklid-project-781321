...
import tensorflow.keras.models, keras.layers, tensorflow.keras.optimizers
...
def AI_trading(dataset,dataset_test): ##modello decisionale sulle previsioni
    dataset.dropna(inplace=True)
...
def AI_trading_minimizecost(dataset,dataset_test):
    from scipy.optimize import minimize
    def costo_ottimizzato(pesi, ARIMA_pred, LSTM_pred, dataset_test):
        predizione_combinata = pesi[0] * ARIMA_pred + pesi[1] * LSTM_pred
        return np.mean((predizione_combinata - dataset_test.Close[:len(ARIMA_prediction(dataset)) + 1]) ** 2)

    dataset.dropna(inplace=True)
    pesi_iniziali = [0.5, 0.5]  # Ottimizzazione dei pesi--> Pesi iniziali
    risultato = minimize(costo_ottimizzato, pesi_iniziali,args=(ARIMA_prediction(dataset), LSTM_prediction(dataset)[:len(ARIMA_prediction(dataset)) + 1], dataset_test),method='Nelder-Mead')
    pesi_ottimali = risultato.x #ottimizzazione dei pesi
...
def AI_trading_NN(dataset, dataset_test):
    ...
    dataset.dropna(inplace=True)
    ARIMA_pred = ARIMA_prediction(dataset)
    LSTM_pred = LSTM_prediction(dataset)[:len(ARIMA_pred)]

    # Ensure the predictions and dataset_test have the same length
    min_length = min(len(ARIMA_pred), len(LSTM_pred), len(dataset_test))
    ARIMA_pred = ARIMA_pred[:min_length]
    LSTM_pred = LSTM_pred[:min_length]
    y_train = dataset_test.Close[len(dataset_test)-min_length:].values

    # Define the neural network model
    input_ARIMA = Input(shape=(1,), name='ARIMA_input')
    input_LSTM = Input(shape=(1,), name='LSTM_input')

    merged = concatenate([input_ARIMA, input_LSTM])
    output = Dense(1, activation='linear', name='output')(merged)

    model = Model(inputs=[input_ARIMA, input_LSTM], outputs=output)
    model.compile(optimizer=Adam(), loss='mean_squared_error')

    # Prepare training data
    X_train = np.vstack([ARIMA_pred, LSTM_pred]).T

    # Train the model
    model.fit([X_train[:, 0], X_train[:, 1]], y_train, epochs=50, batch_size=32)

    # Predict using the trained model
    predictions = model.predict([ARIMA_pred, LSTM_pred]).flatten()

    dataset_test = dataset_test.iloc[len(dataset_test)-min_length:]
    dataset_test.loc[len(dataset_test)-min_length:,'pred Close'] = predictions
    dataset_test.loc[len(dataset_test)-min_length:,'next_week_pred_return']= dataset_test.loc[len(dataset_test)-min_length:,'pred Close'].shift(1) / dataset_test.loc[len(dataset_test)-min_length:,'pred Close']- 1
    dataset_test.loc[len(dataset_test)-min_length:,'model choice'] = 0  # Initialize the column with zeros

    for x in range(len(dataset_test)-min_length, len(dataset_test)):
        if dataset_test.loc[dataset_test.index[0]+x, 'next_week_pred_return'] < -0.5:
            dataset_test.loc[dataset_test.index[0]+x, 'model choice'] = -1
        elif dataset_test.loc[dataset_test.index[0]+x, 'next_week_pred_return'] > 0.5:
            dataset_test.loc[dataset_test.index[0]+x, 'model choice'] = 1

    return dataset_test

def cumulative_return(amzn,predicted_labels_train, predicted_labels_test): #amazon
    #########################################################################################
    '''
    Calculate the total cumulative return by first obtaining the cumulative return for all predicted bullish weeks,
    and the cumulative return for all predicted bearish weeks. Dividing by 100 because the return is stored as a percentage in the dataframe. The long and short return are summed, 1 is added to get growth coefficient
    (e.g. 2% corresponds to 1.02), all growth coefficients are multiplied, and then 1 is subtracted again.
    '''

    # Combine predictions into a single array
    predicted_labels = np.concatenate([predicted_labels_train, predicted_labels_test])

    # Create a DataFrame for the returns calculation
    amzn['Prediction'] = np.concatenate([predicted_labels_train, predicted_labels_test])

    # Long positions (bullish, prediction == 1)
    amzn['Long_Return'] = (amzn['next_week_return'] / 100) * (amzn['Prediction'] == 1)

    # Short positions (bearish, prediction == -1)
    amzn['Short_Return'] = (-amzn['next_week_return'] / 100) * (amzn['Prediction'] == -1)

    # Total portfolio returns (sum of long and short returns)
    amzn['Portfolio_Return'] = amzn['Long_Return'] + amzn['Short_Return']

    # Calculate cumulative returns
    amzn['Cumulative_Return'] = (1 + amzn['Portfolio_Return']).cumprod() - 1

    # Calculate high water mark
    amzn['High_Water_Mark'] = amzn['Cumulative_Return'].cummax()
    ...
    ax.axvline(x=amzn['Date'].iloc[split_idx], color='darkgrey', linestyle='--', label='Train/Validation Split')

    # Plot cumulative returns and high water mark
    ax.plot(amzn['Date'], amzn['Cumulative_Return'], label='Cumulative Return', color='darkgreen', linewidth=1.5)
    ax2 = ax.twinx()
    ax2.plot(amzn['Date'], amzn['Close'], label='Close price', color='#3498db', alpha=0.5, linewidth=1)
    ax.set_title('AMZN Long-Short Equity Strategy Performance')
    ax.set_xlabel('Date')
    ax.legend()
    plt.show()
    plt.savefig('amzn_combined_return.png', transparent=True, dpi=300)
...
def __main__():
    ...
    for dataset, name in zip(dfs, dfs_names):
        print(name)
        hurst_coeff(dataset)
    ...
    amazon_df_test = AI_trading_NN(amazon_df, amazon_df_test) #must do this as the function is not modifying the original dataset itself
    print('Amazon: \n',confusion_matrix(amazon_df_test['real good choice'], amazon_df_test['model choice'], labels=[1, 0, -1]))
    cumulative_return(amazon_df,amazon_df[:int(0.9 * amazon_df.shape[0])]['real good choice'], amazon_df_test['model choice'])



    # cac
    AI_trading_NN(cac_df, cac_df_test)
    cac_df_test = AI_trading_NN(cac_df, cac_df_test)
    print('cac: \n',
          confusion_matrix(cac_df_test['real good choice'], cac_df_test['model choice'], labels=[1, 0, -1]))

    # ibm
    AI_trading_NN(ibm_df, ibm_df_test)
    ibm_df_test = AI_trading_NN(ibm_df, ibm_df_test)
    print('ibm: \n',
    ...
    microsoft_df_test = AI_trading_NN(microsoft_df, microsoft_df_test)
    print('microsoft: \n',
    ...
    nasdaq_df_test = AI_trading_NN(nasdaq_df, nasdaq_df_test)
    print('nasdaq: \n',
    ...
    sp500_df_test = AI_trading_NN(sp500_df, sp500_df_test)
    print('sp500: \n',
...
#try: not to take a cronological validation/test set, but random!
#results: issues with both models.

def ARIMA_prediction_2(df_train, df_test):
    ...
    model = ARIMA(df_train['Close'], order=(2, 2, 1))
    results = model.fit()
    start = len(df_train)
    end = start + len(df_test) - 1
    prediction = results.predict(start=start, end=end, dynamic=False)
    return prediction


def LSTM_prediction_2(dataset):
    def LSTM_third():
        model = Sequential()
        model.add(
            LSTM(units=50,
    ...
    for i in range(15, len(dataset)):
            X.append(dataset.iloc[i - 15:i, [4, 6, 7, 8, 9]].values)
            y.append(dataset.iloc[i, 4])
    X, y = np.array(X), np.array(y)

    #mascherare nan valules
    valid_mask_X = ~np.isnan(X).any(axis=(1, 2))
    ...
    model.fit(X_train, y_train, epochs=10, batch_size=30, validation_split=0.1)
...
def AI_trading_NN_2(dataset, dataset_test,dfs):

    for dataset in dfs:
        # First split to create train and temp datasets
        df_train, df_test = train_test_split(dataset, train_size=0.9, random_state=42, shuffle=True)
        ARIMA_pred = ARIMA_prediction_2(df_train, df_test)

    LSTM_pred = LSTM_prediction_2(df_train, df_test)
    ...
    return dataset_test #modificare opportunamente
...
def __main__2():
    ...
    #amazon
    AI_trading_NN_2(amazon_df, amazon_df_test, dfs)
    amazon_df_test = AI_trading_NN_2(amazon_df, amazon_df_test,dfs) #must do this as the function is not modifying the original dataset itself
    print('Amazon: \n',confusion_matrix(amazon_df_test['real good choice'], amazon_df_test['model choice'], labels=[1, 0, -1]))

    # cac
    AI_trading_NN_2(cac_df, cac_df_test, dfs)
    cac_df_test = AI_trading_NN_2(cac_df, cac_df_test,dfs)
    print('cac: \n',
          confusion_matrix(cac_df_test['real good choice'], cac_df_test['model choice'], labels=[1, 0, -1]))

    # ibm
    AI_trading_NN_2(ibm_df, ibm_df_test,dfs)
    ibm_df_test = AI_trading_NN_2(ibm_df, ibm_df_test,dfs)
    print('ibm: \n',
          confusion_matrix(ibm_df_test['real good choice'], ibm_df_test['model choice'], labels=[1, 0, -1]))

    # microsoft
    AI_trading_NN_2(microsoft_df, microsoft_df_test,dfs)
    microsoft_df_test = AI_trading_NN_2(microsoft_df, microsoft_df_test,dfs)
    print('microsoft: \n',
          confusion_matrix(microsoft_df_test['real good choice'], microsoft_df_test['model choice'], labels=[1, 0, -1]))

    #nasdaq
    AI_trading_NN_2(nasdaq_df, nasdaq_df_test, dfs)
    nasdaq_df_test = AI_trading_NN_2(nasdaq_df, nasdaq_df_test,dfs)
    print('nasdaq: \n',
          confusion_matrix(nasdaq_df_test['real good choice'], nasdaq_df_test['model choice'], labels=[1, 0, -1]))

    # sp500
    AI_trading_NN_2(sp500_df, sp500_df_test,dfs)
    sp500_df_test = AI_trading_NN_2(sp500_df, sp500_df_test,dfs)
    print('sp500: \n',
          confusion_matrix(sp500_df_test['real good choice'], sp500_df_test['model choice'], labels=[1, 0, -1]))
